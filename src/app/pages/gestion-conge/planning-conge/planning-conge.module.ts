import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';

import { PlanningCongeRoutingModule } from './planning-conge-routing.module';
import { AddOrUpdatePlanningCongeComponent } from './add-or-update-planning-conge/add-or-update-planning-conge.component';
import { ListPlanningCongeComponent } from './list-planning-conge/list-planning-conge.component';
import { DetailsPlanningCongeComponent } from './details-planning-conge/details-planning-conge.component';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';
import { BreadcrumbsModule } from '../../../../@fury/shared/breadcrumbs/breadcrumbs.module';
import { ListModule } from '../../../../@fury/shared/list/list.module';
import { MaterialModule } from '../../../../@fury/shared/material-components.module';
import { FurySharedModule } from '../../../../@fury/fury-shared.module';
import { FuryCardModule } from '../../../../@fury/shared/card/card.module';
import { MatTabsModule } from '@angular/material/tabs';
import { PageLayoutDemoContentModule } from '../../page-layouts/components/page-layout-content/page-layout-demo-content.module';
import { MatExpansionModule } from '@angular/material/expansion';


@NgModule({
  declarations: [AddOrUpdatePlanningCongeComponent, ListPlanningCongeComponent, DetailsPlanningCongeComponent],
  imports: [
    CommonModule,
    PlanningCongeRoutingModule,
    FormsModule,
    MaterialModule,
    FurySharedModule,
    FuryCardModule,
    MatTabsModule,
    PageLayoutDemoContentModule,
    MatExpansionModule,

    // Core
    ListModule,
    BreadcrumbsModule,
    ReactiveFormsModule,
  ]
})
export class PlanningCongeModule { }
